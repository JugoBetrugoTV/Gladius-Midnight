...
layout.defaultSettings = {
    ...
    textures          = {
        generalStatusBarTexture       = "Gladius Default",
        healStatusBarTexture          = "Gladius Stripes",
...
function GladiusFrameMixin:AddPixelBorderToFrame()
    ...
    local raidLS = self.parent.db.profile.layoutSettings[self.parent.db.profile.currentLayout]
    if raidLS and raidLS.showDispels == false then
        borders.dispel:Hide()
    end
...
function GladiusMixin:RemovePixelBorders()
...
function layout:Initialize(frame)
    ...
    GladiusMixin.useSpecClassIcon = true
    ...
    if (frame:GetID() == GladiusMixin.maxArenaOpponents) then
        ...
        for n = 1, #GladiusMixin.drCategories do
            local drFrame = frame[GladiusMixin.drCategories[n]]
    ...
    if not frame.Trinket.TrinketPixelBorderHook then
        hooksecurefunc(frame.Trinket.Texture, "SetTexture", function(self, t)
            if not GladiusMixin.showPixelBorder then
    ...
    if not frame.Racial.RacialPixelBorderHook then
        hooksecurefunc(frame.Racial.Texture, "SetTexture", function(self, t)
            if not t or not GladiusMixin.showPixelBorder then
    ...
    if not frame.Dispel.DispelPixelBorderHook then
        hooksecurefunc(frame.Dispel.Texture, "SetTexture", function(self, t)
            local raidLS2 = frame.parent.db.profile.layoutSettings[frame.parent.db.profile.currentLayout]
            local dispelsOn = not raidLS2 or raidLS2.showDispels ~= false
    ...
    local raidLS3 = frame.parent.db.profile.layoutSettings[frame.parent.db.profile.currentLayout]
    if raidLS3 and raidLS3.showDispels == false then
        frame.PixelBorders.dispel:Hide()
    end
